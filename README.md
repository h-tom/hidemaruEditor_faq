...
-   [秀丸エディタの各ＯＳ毎の最新版](s_saisin.html) を変更。
-   [秀丸エディタVer.8.00 マクロ関連の変更履歴](3_macrov8.html)を変更。
-   [第VI部～秀丸エディタ拡張モジュール　秀丸エディタ持ち出しキットについて](6_takeout.html)を追加。
...
-   [第Ⅰ部～秀丸エディタＱ＆Ａ集　Ｑ＆Ａ集の表記について](./s_hyouki.html)を修正。設定について追記。
-   [第Ⅰ部～秀丸エディタＱ＆Ａ集　よくある質問](./yokuaru.html)を修正。行がつながってしまっていた箇所があったので修正。
...
-   [第VI部～秀丸エディタ拡張モジュール　秀丸パブリッシャーについて](./6_publisher.html)を変更。64bit版を対象にした説明である事を追記。
...
-   [第IV部～テキスト編集を極める！！　起動オプションを使ってみよう](./4_cmdlin.html)を修正。バッチファイル修正。
    (for文の箇所を、コマンドプロンプトで実行させると、終了まで待機しないので、start で終了待ちするように修正)
